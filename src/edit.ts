...
function fromGrid(doc: Doc, wall: WallSegment): Draggable | undefined {
   ...
   */
  const anchored: SpaceForm[] = [];
  for (const kind of ["rooms", "outdoor"] as const) {
    const group = asObj(doc[kind]);
    if (!group) continue;
    for (const id of Object.keys(group)) {
      const form = spaceForm(doc, kind, id);
      if (form && form.vertices.some((p) => near(p[axis], wall.c))) anchored.push(form);
    }
  }
  const carry = (next: number) => anchored.flatMap((f) => f.edge(axis as 0 | 1, wall.c, next));
